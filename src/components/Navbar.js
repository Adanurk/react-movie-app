      ...
      <nav className='navbar navbar-expand-lg'>
        <div className='container-fluid'>
         ...
         {currentUser ? (
          ...
          {currentUser ? (<button className='ms-2 btn btn-outline-light' onClick={() => logOut()}>Logout</button>) : (<button className='ms-2 btn btn-outline-light' onClick={() => navigate("/register")}>Register</button>)} 
          </div>
        </div>
      </nav>
    </div>
